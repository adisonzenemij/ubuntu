...
# Script: Servicio SSH
# Objetivo:
#   Administrar el servicio SSH desde un menú interactivo.
...
install_service_flow() {
  ...
  echo
  echo "Instalación del servicio SSH finalizada."
}
...
configure_service_port() {
  ...
  if ! ask_yes_no "¿Confirmas configurar el puerto $new_port para SSH?"; then
    echo "Operación cancelada."
    return 0
  fi
  ...
  if $SUDO_CMD grep -Eq '^#?Port[[:space:]]+' "$CONFIG_FILE"; then
    run_cmd $SUDO_CMD sed -i "s/^#\?Port[[:space:]].*/Port ${new_port}/" "$CONFIG_FILE"
  else
    echo "Port ${new_port}" | $SUDO_CMD tee -a "$CONFIG_FILE" >/dev/null
  fi

  if ! $SUDO_CMD sshd -t; then
    echo "La configuración SSH tiene errores. Se abre nano para corregir."
    $SUDO_CMD nano "$CONFIG_FILE"
    until $SUDO_CMD sshd -t; do
...
main() {
  ...
  while true; do
    ...
    case "$option" in
      ...
      0) echo "Saliendo del menú del servicio SSH."; exit 0 ;;
